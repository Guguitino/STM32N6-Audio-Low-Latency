...
static AudioBM_play_back_t audio_playback_ctx;

static AudioBM_proc_t audio_ll_ctx;
#endif
#if (CTRL_X_CUBE_AI_AUDIO_OUT==COM_TYPE_HEADSET)
int16_t  proc_buf[PROC_BUFFER_SIZE] __NON_CACHEABLE;
int16_t playback_buf[PLAYBACK_BUFFER_SIZE] __NON_CACHEABLE;

int16_t  ll_playback_buf[1280*4] __NON_CACHEABLE;
#endif
...
void exec_bm(void)
{
	bool cont = true;
	time_stats_init();
	initAudioPlayBack(&audio_playback_ctx);
	...
	while(cont)
	{
		...
		uint16_t	audio_acq_len = AUDIO_ACQ_LEN;
...
void initAudioProc(AudioBM_proc_t * ctx_ptr)
{
	...
	ctx_ptr->audioPlayBackCtx.ring_buff.readSampleIndex  = 0 ;
	ctx_ptr->audioPlayBackCtx.ring_buff.writeSampleIndex = 0 ;
	ctx_ptr->audioPlayBackCtx.ring_buff.nbFrames = 1;
	ctx_ptr->audioPlayBackCtx.ring_buff.nbBytesPerSample = 2;
	ctx_ptr->audioPlayBackCtx.ring_buff.nbSamples = PROC_BUFFER_SIZE;
	ctx_ptr->audioPlayBackCtx.ring_buff.pData = (uint8_t *) proc_buf;
	ctx_ptr->audioPlayBackCtx.cnt = 0 ;
}
 ...
 */
bool audio_process(AudioBM_acq_t * acq_ctx_ptr,AudioBM_proc_t * proc_ctx_ptr, uint16_t audio_acq_len)
{
	...
	AudioCapture_ring_buff_feed(&audio_proc_ctx.audioPlayBackCtx.ring_buff, (uint8_t *) audioPtr , audio_acq_len);

	if (audio_playback_ctx.cnt== 1)
	{
		/* Start the playback */
		if (BSP_ERROR_NONE != BSP_AUDIO_OUT_Play(0, audio_playback_ctx.ring_buff.pData, \
				PLAYBACK_BUFFER_SIZE * sizeof(int16_t)))
		{
			Error_Handler();
		}
	}
	audio_playback_ctx.cnt++;
#endif
...
{
	assert(Instance==0);
	AudioCapture_ring_buff_consume_no_cpy(&audio_playback_ctx.ring_buff, PLAYBACK_BUFFER_SIZE/2);

	uint8_t pass_buff[PLAYBACK_BUFFER_SIZE/2];
	AudioCapture_ring_buff_consume(pass_buff, &audio_proc_ctx.audioPlayBackCtx.ring_buff, PLAYBACK_BUFFER_SIZE/2);
	AudioCapture_ring_buff_feed(&audio_playback_ctx.ring_buff, pass_buff, PLAYBACK_BUFFER_SIZE/2);
}
