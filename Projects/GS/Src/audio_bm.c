...
static void AudioPlayBack(AudioBM_play_back_t *ctx_ptr, int16_t *pData, \
                                                uint16_t nbSamples, uint32_t ring_size_buffer);
...
static AudioBM_play_back_t audio_llplayback_ctx;
static uint8_t llacq_buf[AUDIO_ACQ_LEN * sizeof(int16_t)];
#endif
#if (CTRL_X_CUBE_AI_AUDIO_OUT==COM_TYPE_HEADSET)
int16_t  playback_buf[PLAYBACK_BUFFER_SIZE] __NON_CACHEABLE;
int16_t  llplayback_buf[LLPLAYBACK_BUFFER_SIZE] __NON_CACHEABLE;
#endif
...
static ring_buff_stats_t ringBuffStatsBuf[2];

void printRingBuffStats(uint32_t Idx)
{
	fflush(stdout);
}
...
bool audio_process_low_latency(AudioBM_acq_t * acq_ctx_ptr, AudioBM_play_back_t * llplayback_ctx_ptr, uint32_t audio_acq_len)
{
	...
	uint32_t idxMax = llplayback_ctx_ptr->ring_buff.nbSamples * llplayback_ctx_ptr->ring_buff.nbFrames;

    uint32_t latency_target_samples = audio_acq_len;
    // --- Contrôle de la latence ---
    if (llplayback_ctx_ptr->ring_buff.availableSamples > latency_target_samples)
    {
        // Trop de backlog → on saute en resynchronisant
        int32_t newRead = (int32_t)llplayback_ctx_ptr->ring_buff.writeSampleIndex
                          - (int32_t)latency_target_samples;

        if (newRead < 0) newRead += idxMax;

        llplayback_ctx_ptr->ring_buff.readSampleIndex  = (uint32_t)newRead;
        llplayback_ctx_ptr->ring_buff.availableSamples = latency_target_samples;
    }

    // --- Acquisition d’un bloc ---
    AudioCapture_ring_buff_consume(llacq_buf, &acq_ctx_ptr->ring_buff, audio_acq_len);

    // --- Playback direct ---
    AudioPlayBack(llplayback_ctx_ptr, (int16_t *)llacq_buf, audio_acq_len, LLPLAYBACK_BUFFER_SIZE);

    ringBuffStatsBuf[0].bufferSize = acq_ctx_ptr->ring_buff.nbSamples * acq_ctx_ptr->ring_buff.nbFrames;
	ringBuffStatsBuf[0].availableSamples = acq_ctx_ptr->ring_buff.availableSamples;
	ringBuffStatsBuf[0].readSampleIndex = acq_ctx_ptr->ring_buff.readSampleIndex;
	ringBuffStatsBuf[0].writeSampleIndex = acq_ctx_ptr->ring_buff.writeSampleIndex;

	ringBuffStatsBuf[1].bufferSize = llplayback_ctx_ptr->ring_buff.nbSamples * llplayback_ctx_ptr->ring_buff.nbFrames;
	ringBuffStatsBuf[1].availableSamples = llplayback_ctx_ptr->ring_buff.availableSamples;
	ringBuffStatsBuf[1].readSampleIndex = llplayback_ctx_ptr->ring_buff.readSampleIndex;
	ringBuffStatsBuf[1].writeSampleIndex = llplayback_ctx_ptr->ring_buff.writeSampleIndex;

    BSP_LED_Toggle(LED_GREEN);  // petit indicateur visuel
...
void exec_bm(void)
{
  ...
  audio_llplayback_ctx.ring_buff.readSampleIndex  = 0 ;
  audio_llplayback_ctx.ring_buff.writeSampleIndex = 0 ;
  audio_llplayback_ctx.ring_buff.nbFrames = 1;
  audio_llplayback_ctx.ring_buff.nbBytesPerSample = 2;
  audio_llplayback_ctx.ring_buff.nbSamples = LLPLAYBACK_BUFFER_SIZE;
  audio_llplayback_ctx.ring_buff.pData = (uint8_t *)llplayback_buf;
  ...
  while(cont)
  {
    ...
    uint32_t audio_acq_len;
	if(AudioProcIsOn)
	{
		audio_acq_len = AUDIO_ACQ_LEN;
	}
	else
	{
		audio_acq_len = LLPLAYBACK_BUFFER_SIZE/4;
	}
    uint32_t availableSamples = audio_acq_ctx.ring_buff.availableSamples;
    if (availableSamples >= audio_acq_len)
    {
    	if(AudioProcIsOn)
		{
    		cont = audio_process(&audio_acq_ctx,&audio_proc_ctx, audio_acq_len);
		}
		else
		{
...
/**
  ...
  */
bool audio_process(AudioBM_acq_t * acq_ctx_ptr,AudioBM_proc_t * proc_ctx_ptr, uint32_t audio_acq_len)
{
  ...
  AudioPlayBack(&proc_ctx_ptr->audioPlayBackCtx, audioPtr , AUDIO_ACQ_LEN, PLAYBACK_BUFFER_SIZE);
#endif
  ...
  ringBuffStatsBuf[0].bufferSize = acq_ctx_ptr->ring_buff.nbSamples * acq_ctx_ptr->ring_buff.nbFrames;
	...
	ringBuffStatsBuf[1].bufferSize = proc_ctx_ptr->audioPlayBackCtx.ring_buff.nbSamples * proc_ctx_ptr->audioPlayBackCtx.ring_buff.nbFrames;
	ringBuffStatsBuf[1].availableSamples = proc_ctx_ptr->audioPlayBackCtx.ring_buff.availableSamples;
	ringBuffStatsBuf[1].readSampleIndex = proc_ctx_ptr->audioPlayBackCtx.ring_buff.readSampleIndex;
	ringBuffStatsBuf[1].writeSampleIndex = proc_ctx_ptr->audioPlayBackCtx.ring_buff.writeSampleIndex;
...
void printCpuStats(void)
{
	...
	my_printf("                    ");
#endif
	printf ("| %-8d|%6.2f%%|%6.2f|%6.2f|%6.2f|\r\n",time_stats_get_cnt(TIME_STAT_AI_PROC),pre_load+ai_load+post_load,pre_load, ai_load, post_load);

	printf ("|Acq ring buffer    | %d| %d| %d| %d|\r\n", ringBuffStatsBuf[0].bufferSize, \
			ringBuffStatsBuf[0].availableSamples, ringBuffStatsBuf[0].readSampleIndex, \
			ringBuffStatsBuf[0].writeSampleIndex);
	printf ("|PlayBack ring buff | %d| %d| %d| %d|\r\n\n", ringBuffStatsBuf[1].bufferSize, \
				ringBuffStatsBuf[1].availableSamples, ringBuffStatsBuf[1].readSampleIndex, \
				ringBuffStatsBuf[1].writeSampleIndex);
...
static void AudioPlayBack(AudioBM_play_back_t *ctx_ptr, int16_t *pData, \
    uint16_t nbSamples, uint32_t ring_buffer_size)
{
	printf("APB: cnt=%d avail=%d\n", ctx_ptr->cnt, ctx_ptr->ring_buff.availableSamples);
